...
def ConvLayer(bottom, filters, size, stride, pad):
    return L.Convolution(bottom, num_output=filters, kernel_size=size, stride=stride, pad=size/2, weight_filler=dict(type='xavier'))
...
def yolo_net(data_lmdb, label_lmdb, batch_size):
    ...
    n.loss = L.YoloLoss(n.fc2, n.label)

    return n.to_proto()
    
with open('examples/yolo/yolo_auto_train.prototxt', 'w') as f:
    f.write(str(yolo_net('examples/yolo/yolo-train-lmdb', 'examples/yolo/yolo-train-label-lmdb', 8)))