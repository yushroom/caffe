...
public:
	...
	static float rmse(const BBox& a, const BBox& b) {
		return sqrtf(pow(a.x-b.x, 2) + pow(a.y-b.y, 2) + pow(a.w-b.w, 2) + pow(a.h-b.h, 2));
	}

private:
	static float Overlap(const Dtype x1, const Dtype w1, const Dtype x2, const Dtype w2) {
		Dtype l1 = x1 - w1/2;
		Dtype l2 = x2 - w2/2;
	...
	static float Intersection(const BBox& a, const BBox& b) {
	...
	static float Union(const BBox& a, const BBox& b) {
